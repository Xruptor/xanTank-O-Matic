--[[
	grid.lua
		Used to create an alignment grid when moving frames.
--]]

local ADDON_NAME, addon = ...
if not addon.core then addon.core = CreateFrame("frame", ADDON_NAME, UIParent) end

local Grid = addon.core

function Grid:LoadAlignGrid()
	local AlignGrid = CreateFrame('Frame', nil, UIParent)
	AlignGrid:SetAllPoints(UIParent)
	local boxSize = 32

	-- Get the current screen resolution, Mid-points, and the total number of lines
	local ResX, ResY = math.floor(UIParent:GetWidth() + 0.5), math.floor(UIParent:GetHeight() + 0.5)
